...
class QueryEngine {
	...
	private LockSet threadRowLocks = new LockSet();
	...
	public ResultSet execute(SQLSelectForUpdateStatement stmt) {
		...
		//System.out.println("RecordVisitorLock");
		RecordVisitorLock method = new RecordVisitorLock(table);
		
		table.visit(method, stmt.getWhereClause());
		threadRowLocks = method.locks;
		//System.out.println("MethodLock:" + method.locks + Thread.currentThread().getName());
		//System.out.println("Thread row locks: "+threadRowLocks.locks + Thread.currentThread().getName());
	...
	public ResultSet execute(SQLCommitStatement stmt) {
		...
		if(threadRowLocks.locks.size() > 2) System.out.println(threadRowLocks.locks.size() );
		threadRowLocks.release();
		//System.out.println("Released thread row locks: "+threadRowLocks+ Thread.currentThread().getName());
		// Unlock table
		tableLocks.release();
