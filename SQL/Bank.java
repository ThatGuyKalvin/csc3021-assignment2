...
class ResultAggregator {
	...
	ResultAggregator(int nr, int np) {
		...
		barrier = new CyclicBarrier(np + 1, new Runnable() {
			public void run() {
				...
				if (round > NUM_WARMUP) { // skip warmup rounds
					...
					System.out.println("Delay is " + f_delay + " secs; throughput is " + String.format("%.3f", r)
							+ "ops/sec of which " + String.format("%.3f", rs) + " sum and " + String.format("%.3f", rc)
							+ " create");
				} else {
...
/*
 * =========================================================== Process
 * definition for processes that will execute queries.
 * ===========================================================
 */
class TestProcess extends Thread {
	...
	private final SQLStatement qlocka, qsum, qsumlock, qbegintx, qcommit, qaccount;
	private final SQLWhereClause where0, where1;
	private final SQLTable t_account;
	private final SQLFieldList f_account;

	TestProcess(Database database_, Table tbl_a, AtomicInteger nxt_a, Vector<Integer> acct, boolean snl, int total_,
			int freq_sum_, int freq_create_, int tid_, ResultAggregator agg_) {
		database = database_;
		tbl_account = tbl_a;
		next_account = nxt_a;
		accounts = acct;
		...
		final SetDataValue qv_accounts = new SetDataValue(qv_account0, qv_account1);
		...
		// Sum all accounts, lock all accounts
		{
			...
			fields.addField(new SQLField(new String("amount"), ResultSet.AGGREGATE_SUM));
		...
		// Lock accounts
		{
			...
			SQLWhereClause where = new SQLWhereClause(new String("aid"), // field name
					qv_accounts, // value (set of 2)
					SQLWhereClause.CMP_IN);
			qlocka = new SQLSelectForUpdateStatement(f_account, t_account, where);
		}
		...
		// Query accounts
		{
			...
			qaccount = new SQLSelectStatement(fields, t_account, where);
		}

		{
			// UPDATE account SET amount = %d WHERE aid = %d;
			where0 = new SQLWhereClause(new String("aid"), // field name
					qv_account0, // account number
					SQLWhereClause.CMP_EQ);
			where1 = new SQLWhereClause(new String("aid"), // field name
			...
			while (!agg.isFinished()) {
				...
				while (!agg.isRoundDone()) {
					if (freq_sum != 0 && (v * 1023) % freq_sum == tid) {
						...
						if (result.count() != 1)
							throw new Exception("Query sum accounts returns " + result.count() + " results");
						...
						if (sum != total)
							throw new Exception("Query sum accounts returned " + sum + " but expected " + total);
						++vs;
						...
						// Expect two results
						if (!result.success())
							throw new Exception("Query accounts " + account0 + " and " + account1 + " failed.");
						if (result.count() != 2)
							throw new Exception("Query accounts " + account0 + " and " + account1 + " returns "
						...
						for (Record row : result) {
							if (row.get(0).toInteger() == account0)
								...
								throw new Exception("Expect values for accounts " + account0 + " and " + account1
										+ ", got " + row.get(0));
						}
						...
						if (!tfresult.success() || tfresult.count() != 1)
							throw new Exception("Transfer of " + transfer + " from " + account0 + " to " + account1
									+ " failed in update 0.");
...
class Bank {
	...
	public static void main(String[] args) {
			...
			while (i < args.length) {
				if (args[i].equals("--nthreads")) {
					...
					if (args[i + 1].equalsIgnoreCase("true"))
						...
						System.err.println("Unknown value '" + args[i + 1] + "' for option --sum_needs_lock");
						errors = true;
					}
					i += 2;
				} else {
		...
		System.out.println("Measuring performance with " + num_threads + " threads." + " Doing " + num_rounds
				+ " rounds of the experiment after " + ResultAggregator.NUM_WARMUP + " warmup" + " round(s) during "
				+ msecs_measured + " milliseconds per round." + " Running SUM query once every " + freq_sum
				+ " queries.");
		...
		for (int i = 0; i < num_threads; ++i) {
			processes[i] = new TestProcess(database, tbl_account, ai_accounts, accounts, sum_needs_lock, sum_accounts,
					freq_sum, freq_create, i, agg);
		}
		...
		System.out.println("Average delay of a trial was " + avg_delay + " seconds.");
